...
template <typename EncoderType, uint8_t PacketMarker = 0,
...
class PacketSerial_ {
 public:
  ...
  typedef std::function<void(const uint8_t* buffer, size_t size)>
      PacketHandlerFunction;

  /// \brief Construct a default PacketSerial_ device.
  PacketSerial_()
      : _receiveBufferIndex(0), _stream(nullptr), _onPacketFunction(nullptr) {}
  ...
  /// \deprecated Use setStream() to configure a non-default port.
  void begin(Stream* stream) __attribute__((deprecated)) { _stream = stream; }
    ...
    while (_stream->available() > 0) {
      ...
      if (data == PacketMarker) {
        if (_onPacketFunction) {
          ...
          size_t numDecoded = EncoderType::decode(
              _receiveBuffer, _receiveBufferIndex, _decodeBuffer);
  ...
  ///         {
  ///             // Send an alert via a pin (e.g. make an overflow LED) or
  ///             return a
  ///             // user-defined packet to the sender.
  ///             //
  ///             // Ultimately you may need to just increase your recieve
  ///             buffer via the
  ///             // template parameters.
  ///         }
  ///     }
  ...
  bool overflow() const { return _recieveBufferOverflow; }
